...
public class ChainedHashTable<K extends Comparable<K>, V> extends HashTable<K, V> {
	...
	private void rehash() {
		...
		Dictionary<K, V> rehashTable =  new ChainedHashTable<K, V>(2 * maxSize);

		Iterator<Entry<K, V>> it = iterator();

		while (it.hasNext()) {
			Entry<K, V> a = it.next();
			rehashTable.insert(a.getKey(), a.getValue());
		}

		this.table = ((ChainedHashTable<K, V>) rehashTable).table;
		this.currentSize = ((ChainedHashTable<K, V>) rehashTable).currentSize;
		this.maxSize *= 2;
	}
	...
	public V remove(K key) {
		// TODO: Left as an exercise.
		V rem = table[this.hash(key)].remove(key);
